...
function startSystem() {
    ...
    state.workers.alpha = createWorker('alpha', './stockfish.alpha.js', 4);
    state.workers.beta = createWorker('beta', './stockfish.beta.js', 4);
...
function createWorker(name, path, multiPV) {
    ...
    code += '    } else if (c === "search" || c.fen) {\n';
    code += '      engine.postMessage("stop");\n';
    code += '      engine.postMessage("position fen " + (c.fen || c));\n';
